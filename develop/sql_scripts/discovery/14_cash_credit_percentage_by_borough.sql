...
-- borough comes from taxi_zone
-- total number of trips in trip_data
-- cash / card trips are in payment_type
...
SELECT 
    ...
    CAST(SUM(CASE WHEN v_payment_type.payment_type_desc = 'Cash' THEN 1 ELSE 0 END) * 100.0 / COUNT(1) AS DECIMAL(5,2)) AS cash_trips_percentage,
    CAST(SUM(CASE WHEN v_payment_type.payment_type_desc = 'Credit card' THEN 1 ELSE 0 END) * 100.0 / COUNT(1) AS DECIMAL(5,2)) AS card_trips_percentage
FROM v_trip_data
